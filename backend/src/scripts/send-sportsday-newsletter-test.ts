...
const MEDAL_PATH = "C:\\Users\\91991\\.gemini\\antigravity-ide\\brain\\9e103e97-f8be-4098-bbd7-ec57e8a3af61\\.user_uploaded\\media_1788009833408.jpg";
const TSHIRT_PATH = "C:\\Users\\91991\\.gemini\\antigravity-ide\\brain\\9e103e97-f8be-4098-bbd7-ec57e8a3af61\\.user_uploaded\\media_1788009884579.png";

async function run() {
  console.log("Preparing Sports Day Celebration Newsletter Test...");

  // Copy images to frontend public folder as backup
  try {
    const frontendPublic = path.resolve(process.cwd(), "../frontend/public/images");
    if (!fs.existsSync(frontendPublic)) fs.mkdirSync(frontendPublic, { recursive: true });
    fs.copyFileSync(MEDAL_PATH, path.join(frontendPublic, "sports-day-medal-real.jpg"));
    fs.copyFileSync(TSHIRT_PATH, path.join(frontendPublic, "mountain-run-tshirt-real.png"));
    console.log("Copied images to frontend public folder");
  } catch (err) {
    console.warn("Could not copy to public folder:", err);
  }

  // Upload to Cloudinary for reliable CDN email rendering
  let medalUrl = "https://res.cloudinary.com/yppcqzt6/image/upload/v1787133746/mountainrun/admin/qqrxyzirjkj6yshgkwgg.png";
  let tshirtUrl = "https://mountainrun.in/images/mountain-run-tshirt-real.png";

  try {
    const medalBase64 = `data:image/jpeg;base64,${fs.readFileSync(MEDAL_PATH).toString("base64")}`;
    const medalUpload = await uploadImageToCloudinary(medalBase64, "mountainrun/newsletter");
    medalUrl = medalUpload.secure_url;
    console.log("Uploaded Medal to Cloudinary:", medalUrl);
  } catch (err) {
    console.warn("Cloudinary upload for medal failed, using fallback:", err);
  }

  try {
    const tshirtBase64 = `data:image/png;base64,${fs.readFileSync(TSHIRT_PATH).toString("base64")}`;
    const tshirtUpload = await uploadImageToCloudinary(tshirtBase64, "mountainrun/newsletter");
    tshirtUrl = tshirtUpload.secure_url;
    console.log("Uploaded T-shirt to Cloudinary:", tshirtUrl);
  } catch (err) {
    console.warn("Cloudinary upload for tshirt failed, using fallback:", err);
  }
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0d1217; border: 1px solid #334155; border-radius: 16px; overflow: hidden;">
                <tr>
                  <td style="padding: 16px; text-align: center;">
                    ...
                    <img src="${medalUrl}" alt="Sports Day Celebration Finisher Medal" width="100%" style="max-width: 460px; height: auto; border-radius: 12px; display: block; margin: 0 auto; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0d1217; border: 1px solid #1e293b; border-radius: 16px; overflow: hidden;">
                <tr>
                  <td style="padding: 16px; text-align: center; border-bottom: 1px solid #1e293b;">
                    ...
                    <img src="${tshirtUrl}" alt="Official DRI-FIT Running Jersey" width="100%" style="max-width: 380px; height: auto; border-radius: 10px; display: block; margin: 12px auto 6px;">
          ...
          <tr>
            <td style="background-color: #0a0e12; padding: 24px; text-align: center; border-top: 1px solid #1e293b;">
              ...
              <p style="margin: 0; font-size: 11px; color: #475569;">
                ...
                <a href="https://mountainrun.in/unsubscribe" style="color: #64748b; text-decoration: underline;">Unsubscribe from updates</a>
              </p>
            </td>
          </tr>
...
`;
  ...
  console.log("Sending test email to itskampitojha@gmail.com...");
